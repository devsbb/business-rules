...
async def run(rule, defined_variables, defined_actions):
    ...
    rule_triggered = await check_conditions_recursively(conditions, defined_variables)
    if rule_triggered:
...
async def check_conditions_recursively(conditions, defined_variables):
    ...
    if keys == ['all']:
        ...
        for condition in conditions['all']:
            is_condition_applicable = await check_conditions_recursively(condition, defined_variables)
            if not is_condition_applicable:
                return False
        return True
        ...
        yield await check_condition(conditions, defined_variables)
